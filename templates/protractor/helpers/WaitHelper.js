 ...
 * @param timeout - you can provide integer value (miliseconds),
 * use one of the predefined values ('small', 'medium', 'long').
 * Defaults to 'medium' timeout equal to 10 seconds.
 * @returns promise
 */
const toBeClickable = (node, name, timeout) => wait(
  until.elementToBeClickable(node),
  name,
  'was not clickable',
  timeout,
);
...
/**
   ...
   * @returns promise
   */
const toBePresent = (node, name, timeout) => wait(
  until.presenceOf(node),
  name,
  'was not present',
  timeout,
);
...
/**
  ...
  * @returns promise
  */
const toBeVisible = (node, name, timeout) => wait(
  until.visibilityOf(node),
  name,
  'was not visible',
  timeout,
);
...
/**
  ...
  * @returns promise
  */
const toBeInvisible = (node, name, timeout) => wait(
  until.invisibilityOf(node),
  name,
  'was not invisible',
  timeout,
);
